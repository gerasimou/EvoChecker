...
public class ConfigurationChecker {
	...
	public static void checkConfiguration() throws EvoCheckerException {
		...
		// Run Configurations > Environment > New > Variable: "
		// + runtimeLibDir +"; Value:" +
		// Constants.MODEL_CHECKING_ENGINE_LIBS_DIR_DEFAULT);
		...
		if (!engineLibs.exists())
			errors.append("Model checking libs at " + engineLibs.getAbsolutePath() + " do not exist!\n" +
					"You can specify the libs in the configuration script using "
					+ Constants.MODEL_CHECKING_ENGINE_LIBS_DIR + "\n");
		else
		...
		// errors.append(Constants.INITIAL_PORT_KEYWORD + " not found in configuration
		// script!\n");
		...
		try {
			...
			System.err.println("EVOCHECKER_TYPE not specified, using NORMAL; Options: "
					+ Arrays.toString(EvoCheckerType.values()));
		...
		try {
			ecEngine = ModelInvokerEngine
					.valueOf(Utility.getPropertyIgnoreNull(Constants.EVOCHECKER_ENGINE).toUpperCase());
			...
			System.err.println("EVOCHECKER_ENGINE not specified, using PRISM; Options: "
					+ Arrays.toString(ModelInvokerEngine.values()));
			Utility.setProperty(Constants.EVOCHECKER_ENGINE, ModelInvokerEngine.PRISM + "");
		}

		File executableEngineFile = null;
		boolean skipEngineCheck = false;
		if (Utility.getProperty(Constants.MODEL_CHECKING_ENGINE, NAN).equals(NAN)) {
			if (ecType == EvoCheckerType.REGION)
				executableEngineFile = new File(Constants.MODEL_CHECKING_ENGINE_REGION);
			else if ((ecType == EvoCheckerType.NORMAL) && (ecEngine == ModelInvokerEngine.STORM)) {
				executableEngineFile = new File(Constants.MODEL_CHECKING_ENGINE_DEFAULT);
				System.err.println(
						"EVOCHECKER_ENGINE specified to Storm. Note that Storm is slower than Prism for model checking concrete instances.");
				...
				executableEngineFile = new File(Constants.MODEL_CHECKING_ENGINE_DEFAULT);
				System.err.println(
						"EVOCHECKER_ENGINE specified to Prism. Note that Prism is slower than Storm for parametric model checking.");
			} else if (ecType == EvoCheckerType.ULTIMATE) {
		...
		if (!skipEngineCheck)
			if (executableEngineFile == null) {
				System.out.println(
						"Warning: the path to the PMC engine is null, despite the following options indicating it should be otherwise:"
								+
								"\nEVOCHECKER_TYPE: " + ecType +
								"\nEVOCHECKER_ENGINE: " + ecEngine +
								"\nPlease make sure they were set correctly.");
			}
		if (!executableEngineFile.exists())
			errors.append(
					"Model Checking engine at " + executableEngineFile.getAbsolutePath() + " does not exist!\n" +
							"You can specify the engine in the configuration script using "
							+ Constants.MODEL_CHECKING_ENGINE_LIBS_DIR + "\n");
		else
			Utility.setProperty(Constants.MODEL_CHECKING_ENGINE, executableEngineFile.getAbsolutePath());
	...
	private static String getConfiguration() {
		...
		str.append("=======  EvoChecker Configuration:  =======\n");
