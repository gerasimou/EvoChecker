...
public class PolynomialUniformMutationAll extends Mutation {
	 ...
	 * Constructor
	 */
	public PolynomialUniformMutationAll(HashMap<String, Object> parameters) {
		super(parameters);
	...
	public void doMutation(Double realProbability, Double intProbability,
		...
		int numRealVars = xReal.getNumberOfDecisionVariables();
		for (int var = 0; var < numRealVars; var++) {
		...
		// Int mutation....
		int numIntegerVars = xInt.getNumberOfDecisionVariables();
		for (int var = 0; var < numIntegerVars; var++) {
			if (PseudoRandom.randDouble() <= intProbability) {
				int newValue = PseudoRandom.randInt(xInt.getLowerBound(var), xInt.getUpperBound(var));
				xInt.setValue(var, newValue);
			}
		}
	} // doMutation
}
